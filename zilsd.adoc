...
|===
|Alignment |Guaranteed atomic? |Can cause misaligned trap?
|8B |Each word access is atomic |no
|4B |Each word access is atomic |yes
|else |no | yes
|===
...
LD and C.LD instructions with destination `x0` are processed as any other load, but the result is discarded entirely.
Specifically, a load pair to `x0` does not cause `x1` to be written.
