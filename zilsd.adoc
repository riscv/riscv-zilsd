...
|===
|Largest alignment |Word accesses guaranteed atomic? |Can cause misaligned trap?
|8B |yes |no
|4B |yes |yes
|else |no | yes
|===

To ensure resumable trap handling is possible for the load instructions, if a trap is taken, the base register must have its original value, while the other register in the pair can have been updated.
